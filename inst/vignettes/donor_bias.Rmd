...
lpv <- list(c(0.25, 0.75))
lsv <- list(c(1, 1))
lgv <- list(list(c(df[1:2,3]),c(df[3:4,3])))
lexpt <- decon_analysis(lgv = lgv, lpv = lpv, lsv = lsv,
                              return.lres = T)
ypb <- lexpt$lres$expt1$lexpt$Y
```

Next, we simulate a new $Z$ signature matrix from a new randomization with the
same amount of donor-specific variation. Using this and the previously generated
...
df.new <- rand_donor_marker_table(ndonor = ndonor, gindexv = gindexv,
                                  sd.offset.pos = offseti,
                                  sd.offset.neg = offseti)
lgv <- list(list(c(df.new[1:2,ndonor+1]),
                 c(df.new[3:4,ndonor+1])))
lexpt.unadj <- decon_analysis(lgv = lgv, lpv = lpv, lsv = lsv,
                              return.lres = T)
zunadj <- lexpt.unadj$lres$expt1$lexpt$Z
punadj <- predtype(Z = zadj, Y = ypb, strict_method = "nnls",
                   proportions = TRUE, verbose = TRUE)
punadj # [1] 0.6020402 0.3979598
```
...
df.adj[,ndonor+2] <- df.adj[,ndonor+2]/varv
lgv <- list(list(c(df.adj[1:2,3]),c(df.adj[3:4,3])))
lexpt.adj <- decon_analysis(lgv = lgv, lpv = lpv, lsv = lsv,
                            return.lres = T)
# get predictions on Y (built from Z_unadj)
yunadj <- lexpt.unadj$lres$expt1$lexpt$Y
zadj <- lexpt.adj$lres$expt1$lexpt$Z
...
```{r}
dft <- data.frame(unadj = punadj, adj = padj, true = unlist(lpv))
