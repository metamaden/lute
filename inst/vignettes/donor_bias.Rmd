...
# grid of scatterplots, corr

dfp$donor <- factor(dfp$donor, levels = dfp$donor)
ggpairs(dfp, columns = seq(ntype), mapping = ggplot2::aes(color = donor))
```

```{r}
dfp$donor <- as.factor(dfp$donor)
flea[,2] <- as.numeric(flea[,2])
ggpairs(flea, columns = 2:4, mapping = aes(fill=species))

ggpairs(dfp, columns = 1:2, mapping = aes(fill=donor))
```
