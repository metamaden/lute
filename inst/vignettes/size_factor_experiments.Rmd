...
We wish to predict $P$ using some $Y$ and $Z$. When the type-specific factors
...
Below, we will show the impact of the $S$ transformation on the $P$ predictions
from deconvolution. We will generate the objects $Z$, $S$, and $P$, then generate 
the pseudobulked $Y$ data as the product of $Z*S*P$. We then predict $P$ using the
two methods of calculating $Z$ (see above). 

We will perform a series of three experiments total where we vary the type-specific 
factors of $S$ in each series. In other words, we vary the relative sizes of each 
type ($s_{1}$ for type 1/$k_{1}$ and $s_{2}$ for type2/$k_{2}$) in the following ways: 
...
## Experiment series A -- cell sizes equal

Specify equal cell sizes as `size1` and `size`, then store these as a list `lsv` 
having the same length as the object `lgv` above. These list lengths correspond 
to the total simulations to be run in the series.
...
We now run deconvolution simulations using the `decon_analysis()` function (see 
`?decon_analysis` for details) and providing the marker gene signals `lgv`, cell 
sizes `lsv`, and proportions `lpv`.
...
## Experiment series B -- type sizes similar

For this series, we set the type-specific sizes to be slightly different. We will
set type 1's size to be 100 and type 2's size to be 95, then run a new simulation 
series. Finally, we will view the summary plots as before.
...
## Experiment series C -- types very different

For this series, we make type 1 much larger than type 2. We will set type 1 to
have size 1000 and type 2 to have size 1, then run and summarize simulations as
above.
...
In experiment series A, we see a validation of the design (a.k.a. "sanity check") 
where results are virtually identical for the two conditions when cell sizes are equal.
In series B, when cell sizes differ just slightly, we see noticeably lower RMSE 
for most simulations in the case of (2), denoted as`zs_transform == TRUE` or the 
teal color in the plots. In series C, when cell sizes are drastically different,
we see even lower RMSE for (2)/`zs_transform == TRUE`. These findings show the 
importance of correcting on a type-specific factor when present in the convoluted
signals matrix $Y$, such as when quantifying expression in bulk tissue RNA-seq data.
