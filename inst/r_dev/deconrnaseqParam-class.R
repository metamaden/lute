...
setMethod("deconvolution", signature(object = "deconrnaseqParam"), function(object){
  requireNamespace("DeconRNASeq"); lparam <- callNextMethod()
  y <- lparam[["y"]]
  z <- lparam[["z"]]
  s <- lparam[["s"]]
  ...
  pred <- result$out.all
  lpred <- lapply(seq(nrow(pred)), function(index){pred[index,]})
  lr <- .parse_deconvolution_predictions_results(lpred, colnames(pred), rownames(pred))
